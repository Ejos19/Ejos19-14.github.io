/**
 * =====================================================================================
 * ARCHIVO: Exportacion.js
 * DESCRIPCIÓN: Módulo integral de exportación para la Cartelera de Enfrentamientos
 *              del Formulario Game.
 *
 * CARACTERÍSTICAS PRINCIPALES:
 *  1. 📄 ORIENTACIÓN DE HOJA SELECCIONABLE (HORIZONTAL / VERTICAL):
 *     - Permite exportar la cartelera en orientación Horizontal (Landscape - Panorámica)
 *       o Vertical (Portrait - Estándar A4).
 ...
 *     - Permite definir imágenes de fondo tanto desde enlaces web (https://...)
 *       como desde carpetas locales de tu proyecto (./assets/fondos/..., ./img/..., etc.).
 *     - Resolución automática de rutas relativas y absolutas para garantizar máxima nitidez.
...
(function (window, document) {
  ...
  // =====================================================================================
  // ⚙️ CONFIGURACIÓN GENERAL DE EXPORTACIÓN Y PARÁMETROS DEL PDF
  // =====================================================================================
  //
  // 📌 GUÍA RÁPIDA DE PERSONALIZACIÓN PARA EL USUARIO:
  //
  // -------------------------------------------------------------------------------------
  // 📄 1. ORIENTACIÓN DE LA HOJA PDF ('landscape' o 'portrait'):
  // -------------------------------------------------------------------------------------
  //  - 'PDF_ORIENTATION':
  //      • "landscape" -> Hoja Horizontal (Panorámica A4, diseño amplio tipo cartelera).
  //      • "portrait"  -> Hoja Vertical (Estándar A4, formato vertical para impresión/carpetas).
  //  - 'PREGUNTAR_ORIENTACION_AL_EXPORTAR':
  //      • true  -> Al hacer clic en "Exportar PDF" se abre un modal visual para que elijas
  //                 si deseas la hoja Horizontal o Vertical en ese momento.
  //      • false -> Genera el PDF directamente usando la orientación definida en 'PDF_ORIENTATION'.
  //
  // -------------------------------------------------------------------------------------
  // 🖼️ 2. CÓMO AGREGAR FONDOS (ENLACES URL O RUTAS LOCALES DEL PROYECTO):
  // -------------------------------------------------------------------------------------
  //  Puedes usar cualquiera de estos dos formatos:
  //    A) ENLACE URL DE INTERNET (Ejemplo: "https://midominio.com/fondos/cancha.jpg")
  //    B) RUTA LOCAL EN TU PROYECTO (Ejemplos):
  //       • "./assets/fondos/fondo_versus.jpg"
  //       • "./img/cancha_baloncesto.png"
  //       • "fondos/estadio_noche.jpg"
  //       • "/imagenes/textura_madera.jpg"
  //
  // -------------------------------------------------------------------------------------
  // 🏀 FONDO 1: DETRÁS DE CADA JUEGO INDIVIDUAL (Banner central del enfrentamiento)
  // -------------------------------------------------------------------------------------
  //  - Variable: 'FONDO_DETRAS_DE_CADA_JUEGO'
  //  - Opacidad: 'OPACIDAD_FONDO_CADA_JUEGO' (de 0.0 a 1.0; 0.85 recomendado para gran nitidez)
  //
  // -------------------------------------------------------------------------------------
  // 🏟️ FONDO 2: DETRÁS DE TODA LA PÁGINA DEL PDF (Fondo global del documento)
  // -------------------------------------------------------------------------------------
  //  - Variable: 'FONDO_GENERAL_PAGINA_PDF'
  //  - Opacidad: 'OPACIDAD_FONDO_GENERAL_PAGINA' (de 0.0 a 1.0; 0.30 recomendado)
  // =====================================================================================

  const CONFIG_EXPORTACION = {
    // ===================================================================================
    // 👈 1. [ORIENTACIÓN] 'landscape' (Horizontal) o 'portrait' (Vertical)
    // ===================================================================================
    ...
    NOMBRE_ARCHIVO_BASE: "Cartelera_Enfrentamientos",
  };

  // Exponer la configuración globalmente para permitir modificaciones dinámicas en consola
  window.CONFIG_EXPORTACION = CONFIG_EXPORTACION;
    ...
    if (btnConfirm) {
      btnConfirm.addEventListener("click", () => {
        ...
        const orientacion = radioSeleccionado
          ...
          : CONFIG_EXPORTACION.PDF_ORIENTATION || "landscape";
        cerrarModal();
  ...
  function iniciarFlujoExportacionPDF() {
    ...
    if (CONFIG_EXPORTACION.PREGUNTAR_ORIENTACION_AL_EXPORTAR) {
      const modal = document.getElementById("pdf-orientation-modal");
      if (modal) {
        // Preseleccionar según la configuración actual
        const currentOrient = CONFIG_EXPORTACION.PDF_ORIENTATION || "landscape";
    ...
    ejecutarExportacionPDF(CONFIG_EXPORTACION.PDF_ORIENTATION || "landscape");
  }
  ...
  // =====================================================================================
  /**
   * Convierte una ruta de imagen (sea URL externa, Base64 o ruta relativa local de carpeta)
   * a una URL válida y absoluta para que html2canvas y las etiquetas CSS la carguen sin fallos.
   */
  function resolverRutaFondo(rutaOFuente) {
    ...
    // Si ya es una URL completa (http, https, data o blob)
    if (/^(https?:\/\/|data:|blob:|\/\/)/i.test(limpia)) {
      return limpia;
    }

    // Si es una ruta local del proyecto, la resolvemos contra la URL actual del navegador
    try {
      const baseUrl = window.location.href.split("#")[0].split("?")[0];
      return new URL(limpia, baseUrl).href;
    } catch (e) {
      console.warn(
        "[Exportacion.js] No se pudo resolver la ruta local:",
        limpia,
        e,
      );
      return limpia;
  ...
  // =====================================================================================
  // 3. OBTENCIÓN Y AGRUPACIÓN DE DATOS DE ENFRENTAMIENTOS
  // =====================================================================================
  /**
   * Obtiene la lista actual de enfrentamientos guardados en la cartelera.
   */
  function obtenerEnfrentamientosPublicados() {
    if (typeof window.getGameMatchups === "function") {
      const list = window.getGameMatchups();
    ...
    try {
      ...
      console.warn("[Exportacion.js] Error leyendo localStorage:", e);
    }
    ...
    matchups.forEach((m) => {
      let rawDate =
        m.gameDate && String(m.gameDate).trim()
          ...
          : "Fecha_No_Definida";
      // Si viene con formato ISO largo (ej: 2026-08-17T04:00:00.000Z), extraer solo YYYY-MM-DD para la agrupación limpia
  ...
  /**
   * Formatea una fecha de juego para mostrar en la cabecera del enfrentamiento
   */
  ...
  /**
   * Genera un timestamp limpio para el nombre de archivo (ej: 20260817_2009)
   */
  ...
  function exportarCSV() {
    ...
    const filename = `${CONFIG_EXPORTACION.NOMBRE_ARCHIVO_BASE}_${obtenerTimestampArchivo()}.csv`;
  ...
  function exportarExcel() {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Cartelera de Juegos");

    const filename = `${CONFIG_EXPORTACION.NOMBRE_ARCHIVO_BASE}_${obtenerTimestampArchivo()}.xlsx`;
  ...
  // =====================================================================================
  /**
   * Ejecuta la generación del PDF con la orientación especificada ('landscape' o 'portrait').
   */
  async function ejecutarExportacionPDF(orientacion = "landscape") {
    const matchups = obtenerEnfrentamientosPublicados();
    if (matchups.length === 0) {
      mostrarNotificacion(
        "No hay enfrentamientos en la cartelera para exportar a PDF",
    ...
    try {
      ...
      console.error("[Exportacion.js] Error exportando PDF:", err);
   ...
   *  - Encabezado limpio con "CARTELERA OFICIAL DE ENFRENTAMIENTOS" (sin etiqueta naranja debajo).
  ...
  async function renderizarYDescargarPdfFecha(
    ...
    const urlFondoGlobal = resolverRutaFondo(
      CONFIG_EXPORTACION.FONDO_GENERAL_PAGINA_PDF,
    );
    const urlFondoVersus = resolverRutaFondo(
      CONFIG_EXPORTACION.FONDO_DETRAS_DE_CADA_JUEGO,
    );
    ...
    printContainer.id = `pdf-export-render-${groupIndex}`;
    ...
    // 2. Estilos CSS idénticos a la cartelera del formulario Game
    ...
    styleEl.textContent = `
      ...
      .pdf-page-global-bg {
        ...
        opacity: ${CONFIG_EXPORTACION.OPACIDAD_FONDO_GENERAL_PAGINA || 0.25};
        z-index: 0;
        pointer-events: none;
      }
      ...
      /* ENCABEZADO SUPERIOR DE LA PÁGINA (Limpio, sin etiqueta naranja debajo) */
      .pdf-top-banner {
        display: flex;
      ...
      /* 1. Barra Superior del Juego: JUEGO 1 | 08:00 PM | 2026-08-17 */
      .pdf-game-header {
        background: #0a0402;
      ...
      .pdf-game-number {
        color: #ff8c00;
        font-weight: 900;
        font-size: ${isPortrait ? "13.5px" : "15px"};
        letter-spacing: 0.05em;
        text-transform: uppercase;
        display: flex;
        align-items: center;
      ...
      .pdf-game-time-badge {
        ...
        padding: 3px 10px;
        border-radius: 6px;
        font-size: ${isPortrait ? "11.5px" : "13px"};
        font-weight: 800;
        font-family: 'JetBrains Mono', monospace;
        display: flex;
        align-items: center;
      ...
      .pdf-game-body {
        ...
        background: transparent; /* Transparente para que el fondo resalte con total nitidez */
      ...
      .pdf-game-bg-image {
        ...
        opacity: ${CONFIG_EXPORTACION.OPACIDAD_FONDO_CADA_JUEGO || 0.85};
        z-index: 0;
        pointer-events: none;
      }
      ...
      /* Tarjetas de Datos del Equipo (Con bordes ámbar y fondo oscuro) */
      .pdf-team-box {
        ...
        min-width: ${isPortrait ? "140px" : "200px"};
        max-width: ${isPortrait ? "240px" : "300px"};
      }
      ...
      /* Logo de cada equipo */
      .pdf-team-logo-wrap {
        width: ${isPortrait ? "85px" : "110px"};
        height: ${isPortrait ? "85px" : "110px"};
        min-width: ${isPortrait ? "85px" : "110px"};
        min-height: ${isPortrait ? "85px" : "110px"};
        display: flex;
        align-items: center;
      ...
      /* =========================================================================
         GRUPO CENTRAL: ESTATUS NEÓN, PÍLDORA VS Y MARCADOR
         ========================================================================= */
      .pdf-vs-group {
        position: relative;
        z-index: 2;
      ...
      /* Píldora de Estatus Neón (Encima del VS) */
      .pdf-status-pill {
        display: inline-flex;
        align-items: center;
        justify-content: center;
      ...
      /* Estatus: Pospuesto (Naranja Neón) */
      ...
      /* Marcador de Puntos (Debajo del VS) */
      .pdf-score-box {
        display: inline-flex;
        align-items: center;
        justify-content: center;
    ...
    const ITEMS_POR_PAGINA = isPortrait
      ? CONFIG_EXPORTACION.JUEGOS_POR_PAGINA_VERTICAL || 2
      : CONFIG_EXPORTACION.JUEGOS_POR_PAGINA_HORIZONTAL || 2;
    ...
    for (let p = 0; p < totalPaginas; p++) {
      ...
      // Encabezado limpio de la página (sin etiqueta naranja debajo)
      content.innerHTML = `
        <div class="pdf-top-banner">
          <div class="pdf-top-title-group">
            <span class="pdf-top-logo-icon">🏀</span>
            <h1 class="pdf-top-heading">${escapeHtml(CONFIG_EXPORTACION.PDF_HEADER_TITLE)}</h1>
          </div>
      ...
      for (let i = startIdx; i < endIdx; i++) {
        ...
        card.innerHTML = `
          <!-- 1. Encabezado de la tarjeta: ID | JUEGO X | HORA | FECHA -->
          ...
          <div class="pdf-game-body">
            ...
            <!-- Centro: Estatus Neón, Píldora VS y Marcador -->
      ...
      bottomBar.innerHTML = `
        ...
        <span>Sistema de Gestión Deportiva • Formulario Game • Orientación: ${isPortrait ? "Vertical" : "Horizontal"}</span>
      `;
    ...
    const filename = `${CONFIG_EXPORTACION.NOMBRE_ARCHIVO_BASE}_${fechaLimpia}_${orientSufijo}_${obtenerTimestampArchivo()}.pdf`;
    pdf.save(filename);
  }
  ...
  function generarPdfFechaFallback(
    ...
    doc.text(
      `${CONFIG_EXPORTACION.PDF_HEADER_TITLE} - FECHA: ${fechaJuego}`,
      12,
      16,
    );
    ...
    const orientSufijo = isPortrait ? "Vertical" : "Horizontal";
    doc.save(
      `${CONFIG_EXPORTACION.NOMBRE_ARCHIVO_BASE}_${fechaLimpia}_${orientSufijo}_${obtenerTimestampArchivo()}.pdf`,
    );
  }
  ...
  window.exportarPDF = window.Exportacion.exportarPDF;
})(window, document);
