...
(function () {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    // Sincronización multi-dispositivo en tiempo real cuando la ventana recupera el foco o visibilidad
    window.addEventListener("focus", () => {
      syncBracketFromCloudSilently();
    });
  ...
  function setupBracketEventListeners() {
    ...
    // Botón Cargar PlayOffs guardados por ID o Torneo (Protegido con Autorización de Administrador)
    const btnLoadBracket = document.getElementById("btn-load-bracket");
    if (btnLoadBracket) {
      btnLoadBracket.addEventListener("click", () => {
        requestAdminAccess(
          "Cargar Cuadro de PlayOffs desde la Hoja PlayOffs y Servidor",
          () => {
    ...
    // Botón guardar cuadro (Protegido con Autorización de Administrador)
    const btnSave = document.getElementById("btn-save-bracket");
    if (btnSave) {
      btnSave.addEventListener("click", () => {
        requestAdminAccess(
          "Guardar Cuadro de PlayOffs en la Hoja PlayOffs y Servidor",
          () => {
  ...
  /**
   * Construye el registro tabular estructurado en UNA SOLA FILA para la hoja "PlayOffs"
   * Contiene todas las llaves (8 Octavos A/B, 4 Cuartos A/B, 2 Semifinales A/B, 1 Final A/B, Campeón y Metadatos)
   * siguiendo estrictamente la estructura solicitada para el ID ("PO-0000001").
   */
  function buildPlayOffsSingleRow(bracket, customId = null) {
    ...
    const champName =
      ...
      (bracket.final?.winner
        ? bracket.final.winner === bracket.final.teamA?.id
          ? bracket.final.teamA?.name
          : bracket.final.teamB?.name
        : "");

    // Helper para nombre de ganador
    const getWinnerTeamName = (match) => {
    ...
    const row = {
      ID: customId || "PO-0000001",
      ID_Tournament: tIdRaw,
      Torneo: tName,
    };

    // 1. OCTAVOS (8 tarjetas: Octavo1 a Octavo8)
    const oct = Array.isArray(bracket.octavos) ? bracket.octavos : [];
    for (let i = 1; i <= 8; i++) {
      const m = oct[i - 1] || {};
      row[`Octavo${i} A`] = m.teamA?.name || "";
      row[`Octavo${i} B`] = m.teamB?.name || "";
      row[`Marcador_Octavo${i} A`] =
        m.scoreA !== "" && m.scoreA !== undefined && m.scoreA !== null
          ? m.scoreA
          : "";
      row[`Marcador_Octavo${i} B`] =
        m.scoreB !== "" && m.scoreB !== undefined && m.scoreB !== null
          ? m.scoreB
          : "";
      row[`Ganador_Octavo${i}`] = getWinnerTeamName(m);
    }

    // 2. CUARTOS (4 tarjetas: Cuartos1 a Cuartos4)
    const qf = Array.isArray(bracket.cuartos) ? bracket.cuartos : [];
    for (let i = 1; i <= 4; i++) {
      const m = qf[i - 1] || {};
      row[`Cuartos${i} A`] = m.teamA?.name || "";
      row[`Cuartos${i} B`] = m.teamB?.name || "";
      row[`Marcador_Cuartos${i} A`] =
        m.scoreA !== "" && m.scoreA !== undefined && m.scoreA !== null
          ? m.scoreA
          : "";
      row[`Marcador_Cuartos${i} B`] =
        m.scoreB !== "" && m.scoreB !== undefined && m.scoreB !== null
          ? m.scoreB
          : "";
      row[`Ganador_Cuartos${i}`] = getWinnerTeamName(m);
    }

    // 3. SEMIFINALES (2 tarjetas: Semifinal1 y Semifinal2)
    const sf = Array.isArray(bracket.semis) ? bracket.semis : [];
    for (let i = 1; i <= 2; i++) {
      const m = sf[i - 1] || {};
      row[`Semifinal${i} A`] = m.teamA?.name || "";
      row[`Semifinal${i} B`] = m.teamB?.name || "";
      row[`Marcador_Semifinal${i} A`] =
        m.scoreA !== "" && m.scoreA !== undefined && m.scoreA !== null
          ? m.scoreA
          : "";
      row[`Marcador_Semifinal${i} B`] =
        m.scoreB !== "" && m.scoreB !== undefined && m.scoreB !== null
          ? m.scoreB
          : "";
      row[`Ganador_Semifinal${i}`] = getWinnerTeamName(m);
    }

    // 4. GRAN FINAL (1 tarjeta: Final1)
    const fin = bracket.final || {};
    row["Final1 A"] = fin.teamA?.name || "";
    row["Final1 B"] = fin.teamB?.name || "";
    row["Marcador_Final1 A"] =
      fin.scoreA !== "" && fin.scoreA !== undefined && fin.scoreA !== null
        ? fin.scoreA
        : "";
    row["Marcador_Final1 B"] =
      fin.scoreB !== "" && fin.scoreB !== undefined && fin.scoreB !== null
        ? fin.scoreB
        : "";
    row["Ganador_Final1"] = getWinnerTeamName(fin) || champName;

    // 5. CAMPEÓN Y METADATOS
    row["Campeon"] = champName;
    row["Fecha_Evento"] = dateVal;
    row["Sede_Evento"] = venueVal;
    row["Ciudad_Evento"] = cityVal;
    row["Fecha_Actualizacion"] = nowIso;

    return row;
  }

  /**
   * Genera el desglose tabular de filas para compatibilidad con listados detallados
   */
  function generatePlayOffsSheetRows(bracket) {
    const singleRow = buildPlayOffsSingleRow(bracket);
    return [singleRow];
  }

  /**
   * Helper para buscar logo de equipo en cache o standings
   */
  function findTeamLogoFromCache(teamId, teamName) {
    ...
    if (afiliadosCache && afiliadosCache.length > 0) {
      const found = afiliadosCache.find((a) => {
        const aId = String(a.ID || a.id || "")
          .trim()
          .toLowerCase();
        const aRif = String(a.RIF || a.rif || "")
          .trim()
          .toLowerCase();
        const aName = String(a.NombreCompleto || a.Nombre || a.nombre || "")
          .trim()
          .toLowerCase();
        return (
          (cleanId && (aId === cleanId || aRif === cleanId)) ||
          (cleanName && aName === cleanName)
        );
      });
      if (found) {
        const raw =
    ...
    if (lastCalculatedStandings && lastCalculatedStandings.length > 0) {
      const foundSt = lastCalculatedStandings.find((s) => {
        ...
        const sName = String(s.name || "")
          ...
          .toLowerCase();
        return (
          (cleanId && sId === cleanId) || (cleanName && sName === cleanName)
        );
      });
  ...
  /**
   * Helper para crear objeto equipo por nombre o ID
   */
  function createTeamObj(teamName, teamId = null) {
    const name = String(teamName || "").trim();
    if (!name) return null;
    const id = teamId ? String(teamId).trim() : name;
    return {
      id: id,
      name: name,
      logo: findTeamLogoFromCache(id, name),
      rawLogo: "",
    };
  }

  /**
   * Reconstruye el estado interactivo del bracket a partir de una fila única de la hoja PlayOffs
   */
  function reconstructBracketFromSingleRow(r) {
    if (!r || typeof r !== "object") return false;
    let reconstructed = false;

    // 1. Reconstruir Octavos (1 a 8)
    for (let i = 1; i <= 8; i++) {
      const idx = i - 1;
      const teamAName = r[`Octavo${i} A`] || r[`Octavo${i}A`] || "";
      const teamBName = r[`Octavo${i} B`] || r[`Octavo${i}B`] || "";
      const scoreA =
        r[`Marcador_Octavo${i} A`] !== undefined
          ? r[`Marcador_Octavo${i} A`]
          : r[`Marcador_Octavo${i}A`] !== undefined
            ? r[`Marcador_Octavo${i}A`]
            : "";
      const scoreB =
        r[`Marcador_Octavo${i} B`] !== undefined
          ? r[`Marcador_Octavo${i} B`]
          : r[`Marcador_Octavo${i}B`] !== undefined
            ? r[`Marcador_Octavo${i}B`]
            : "";
      const winnerName = r[`Ganador_Octavo${i}`] || "";

      if (bracketState.octavos && bracketState.octavos[idx]) {
        if (teamAName)
          bracketState.octavos[idx].teamA = createTeamObj(teamAName);
        if (teamBName)
          bracketState.octavos[idx].teamB = createTeamObj(teamBName);
        bracketState.octavos[idx].scoreA = scoreA;
        bracketState.octavos[idx].scoreB = scoreB;
        if (winnerName) {
          const wId =
            winnerName === teamAName && bracketState.octavos[idx].teamA
              ? bracketState.octavos[idx].teamA.id
              : winnerName === teamBName && bracketState.octavos[idx].teamB
                ? bracketState.octavos[idx].teamB.id
                : winnerName;
          bracketState.octavos[idx].winner = wId;
        }
        reconstructed = true;
      }
    }

    // 2. Reconstruir Cuartos (1 a 4)
    for (let i = 1; i <= 4; i++) {
      const idx = i - 1;
      const teamAName = r[`Cuartos${i} A`] || r[`Cuartos${i}A`] || "";
      const teamBName = r[`Cuartos${i} B`] || r[`Cuartos${i}B`] || "";
      const scoreA =
        r[`Marcador_Cuartos${i} A`] !== undefined
          ? r[`Marcador_Cuartos${i} A`]
          : r[`Marcador_Cuartos${i}A`] !== undefined
            ? r[`Marcador_Cuartos${i}A`]
            : "";
      const scoreB =
        r[`Marcador_Cuartos${i} B`] !== undefined
          ? r[`Marcador_Cuartos${i} B`]
          : r[`Marcador_Cuartos${i}B`] !== undefined
            ? r[`Marcador_Cuartos${i}B`]
            : "";
      const winnerName = r[`Ganador_Cuartos${i}`] || "";

      if (bracketState.cuartos && bracketState.cuartos[idx]) {
        if (teamAName)
          bracketState.cuartos[idx].teamA = createTeamObj(teamAName);
        if (teamBName)
          bracketState.cuartos[idx].teamB = createTeamObj(teamBName);
        bracketState.cuartos[idx].scoreA = scoreA;
        bracketState.cuartos[idx].scoreB = scoreB;
        if (winnerName) {
          const wId =
            winnerName === teamAName && bracketState.cuartos[idx].teamA
              ? bracketState.cuartos[idx].teamA.id
              : winnerName === teamBName && bracketState.cuartos[idx].teamB
                ? bracketState.cuartos[idx].teamB.id
                : winnerName;
          bracketState.cuartos[idx].winner = wId;
        }
        reconstructed = true;
      }
    }

    // 3. Reconstruir Semifinales (1 y 2)
    for (let i = 1; i <= 2; i++) {
      const idx = i - 1;
      const teamAName = r[`Semifinal${i} A`] || r[`Semifinal${i}A`] || "";
      const teamBName = r[`Semifinal${i} B`] || r[`Semifinal${i}B`] || "";
      const scoreA =
        r[`Marcador_Semifinal${i} A`] !== undefined
          ? r[`Marcador_Semifinal${i} A`]
          : r[`Marcador_Semifinal${i}A`] !== undefined
            ? r[`Marcador_Semifinal${i}A`]
            : "";
      const scoreB =
        r[`Marcador_Semifinal${i} B`] !== undefined
          ? r[`Marcador_Semifinal${i} B`]
          : r[`Marcador_Semifinal${i}B`] !== undefined
            ? r[`Marcador_Semifinal${i}B`]
            : "";
      const winnerName = r[`Ganador_Semifinal${i}`] || "";

      if (bracketState.semis && bracketState.semis[idx]) {
        if (teamAName) bracketState.semis[idx].teamA = createTeamObj(teamAName);
        if (teamBName) bracketState.semis[idx].teamB = createTeamObj(teamBName);
        bracketState.semis[idx].scoreA = scoreA;
        bracketState.semis[idx].scoreB = scoreB;
        if (winnerName) {
          const wId =
            winnerName === teamAName && bracketState.semis[idx].teamA
              ? bracketState.semis[idx].teamA.id
              : winnerName === teamBName && bracketState.semis[idx].teamB
                ? bracketState.semis[idx].teamB.id
                : winnerName;
          bracketState.semis[idx].winner = wId;
        }
        reconstructed = true;
      }
    }

    // 4. Reconstruir Gran Final
    const finalAName = r["Final1 A"] || r["Final1A"] || r["Final A"] || "";
    const finalBName = r["Final1 B"] || r["Final1B"] || r["Final B"] || "";
    const finalScoreA =
      r["Marcador_Final1 A"] !== undefined
        ? r["Marcador_Final1 A"]
        : r["Marcador_Final A"] !== undefined
          ? r["Marcador_Final A"]
          : "";
    const finalScoreB =
      r["Marcador_Final1 B"] !== undefined
        ? r["Marcador_Final1 B"]
        : r["Marcador_Final B"] !== undefined
          ? r["Marcador_Final B"]
          : "";
    const finalWinnerName =
      r["Ganador_Final1"] || r["Ganador_Final"] || r["Campeon"] || "";

    if (bracketState.final) {
      if (finalAName) bracketState.final.teamA = createTeamObj(finalAName);
      if (finalBName) bracketState.final.teamB = createTeamObj(finalBName);
      bracketState.final.scoreA = finalScoreA;
      bracketState.final.scoreB = finalScoreB;
      if (finalWinnerName) {
        const wId =
          finalWinnerName === finalAName && bracketState.final.teamA
            ? bracketState.final.teamA.id
            : finalWinnerName === finalBName && bracketState.final.teamB
              ? bracketState.final.teamB.id
              : finalWinnerName;
        bracketState.final.winner = wId;
      }
      reconstructed = true;
    }

    // 5. Campeón
    const champName = r["Campeon"] || finalWinnerName;
    if (champName) {
      bracketState.champion = createTeamObj(champName);
    }
  ...
  function reconstructBracketFromRows(rows) {
    ...
    // Si es un formato de fila única con columnas como "Octavo1 A" o "Octavo1 B"
    const firstRow = rows[0];
    if (
      firstRow &&
      (firstRow["Octavo1 A"] !== undefined ||
        firstRow["Octavo1 B"] !== undefined ||
        firstRow["Cuartos1 A"] !== undefined ||
        firstRow["Campeon"] !== undefined ||
        firstRow["ID"])
    ) {
      const okSingle = reconstructBracketFromSingleRow(firstRow);
    ...
    rows.forEach((r) => {
      ...
      if (r.EquipoA_ID || r.EquipoA_Nombre) {
        teamAObj = createTeamObj(
          r.EquipoA_Nombre || `Equipo ${r.EquipoA_ID}`,
          r.EquipoA_ID,
        );
      }
      ...
      if (r.EquipoB_ID || r.EquipoB_Nombre) {
        teamBObj = createTeamObj(
          r.EquipoB_Nombre || `Equipo ${r.EquipoB_ID}`,
          r.EquipoB_ID,
        );
      }
      ...
      if (
        ...
        if (idx >= 0 && idx < bracketState.octavos.length) {
          if (teamAObj) bracketState.octavos[idx].teamA = teamAObj;
          if (teamBObj) bracketState.octavos[idx].teamB = teamBObj;
      ...
      // 2. Cuartos
        ...
        if (idx >= 0 && idx < bracketState.cuartos.length) {
          if (teamAObj) bracketState.cuartos[idx].teamA = teamAObj;
          if (teamBObj) bracketState.cuartos[idx].teamB = teamBObj;
      ...
      // 3. Semifinales
        ...
        if (idx >= 0 && idx < bracketState.semis.length) {
          if (teamAObj) bracketState.semis[idx].teamA = teamAObj;
          if (teamBObj) bracketState.semis[idx].teamB = teamBObj;
      ...
      // 4. Gran Final
        ...
        if (teamAObj) bracketState.final.teamA = teamAObj;
        if (teamBObj) bracketState.final.teamB = teamBObj;
        bracketState.final.scoreA =
          r.Marcador_EquipoA !== undefined ? r.Marcador_EquipoA : "";
        bracketState.final.scoreB =
          r.Marcador_EquipoB !== undefined ? r.Marcador_EquipoB : "";
        ...
        if (r.Campeon_ID || r.Campeon_Nombre) {
          bracketState.champion = createTeamObj(
            r.Campeon_Nombre || r.Ganador_Nombre || "",
            r.Campeon_ID,
          );
        }
        reconstructed = true;
  ...
  /**
   * Inicializa y enlaza los controles del selector de IDs de PlayOffs
   * Permite elegir un ID existente (para sobreescribir) o generar un nuevo ID correlativo ("PO-0000001")
   */
  let availablePlayOffIds = [];

  function initPlayOffsIdControls() {
    const idSelect = document.getElementById("bracket-id-select");
    const idInput = document.getElementById("bracket-id-input");
    const btnRefresh = document.getElementById("btn-refresh-playoff-id");
    const modeBadge = document.getElementById("bracket-id-mode-badge");

    if (idSelect && idInput) {
      idSelect.addEventListener("change", () => {
        const val = idSelect.value;
        if (val === "__NEW__") {
          fetchNextPlayOffId();
          if (modeBadge) {
            modeBadge.className = "bracket-id-badge is-new";
            modeBadge.innerText = "NUEVO ID";
            modeBadge.title = "Se creará un nuevo registro en la hoja PlayOffs";
          }
        } else {
          idInput.value = val;
          if (modeBadge) {
            modeBadge.className = "bracket-id-badge is-overwrite";
            modeBadge.innerText = "SOBREESCRIBIR";
            modeBadge.title = `Al guardar se sobreescribirá la fila existente con ID: ${val}`;
          }
        }
      });
      ...
      idInput.addEventListener("input", () => {
        ...
        const exists = availablePlayOffIds.some(
          (item) => item.id.toUpperCase() === val,
        );
        if (modeBadge) {
          if (exists) {
            modeBadge.className = "bracket-id-badge is-overwrite";
            modeBadge.innerText = "SOBREESCRIBIR";
            modeBadge.title = `El ID ${val} ya existe. Se sobreescribirá la fila existente.`;
          } else {
            modeBadge.className = "bracket-id-badge is-new";
            modeBadge.innerText = "NUEVO ID";
            modeBadge.title = `El ID ${val} no existe. Se creará una fila nueva.`;
          }
        }
      });
    }
    ...
    if (btnRefresh) {
      btnRefresh.addEventListener("click", () => {
        fetchPlayOffIdsList(true);
      });
    }

    // Carga inicial de la lista de IDs disponibles
    fetchPlayOffIdsList(false);
  }

  /**
   * Consulta el siguiente ID correlativo desde el backend (PO-0000001)
   */
  async function fetchNextPlayOffId() {
    const idInput = document.getElementById("bracket-id-input");
    try {
      const res = await fetch(`/api/bracket/next-id?t=${Date.now()}`);
      if (res.ok) {
        const data = await res.json();
        if (data && data.nextId && idInput) {
      ...
      console.warn(
        "[PositionTable.js] Falló consulta de siguiente ID en servidor:",
        e,
      );
    }
    if (idInput && !idInput.value) {
      idInput.value = "PO-0000001";
    }
    return idInput ? idInput.value : "PO-0000001";
  }

  /**
   * Consulta todos los IDs de PlayOffs guardados para poblar el dropdown selector
   */
  async function fetchPlayOffIdsList(showToastNotify = false) {
    const idSelect = document.getElementById("bracket-id-select");
    const idInput = document.getElementById("bracket-id-input");
    const btnRefresh = document.getElementById("btn-refresh-playoff-id");
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data && Array.isArray(data.items)) {
          availablePlayOffIds = data.items;
          populatePlayOffIdDropdown(data.items);
          if (showToastNotify) {
            showToast(
              `Lista de IDs de PlayOffs actualizada (${data.items.length} IDs encontrados).`,
              "info",
            );
          }
        }
      ...
      console.warn(
        "[PositionTable.js] Falló carga de lista de IDs de PlayOffs:",
        e,
      );
    } finally {
      if (btnRefresh) {
  ...
  function populatePlayOffIdDropdown(items) {
    const idSelect = document.getElementById("bracket-id-select");
    const idInput = document.getElementById("bracket-id-input");
    if (!idSelect) return;

    const currentVal = idSelect.value;
    idSelect.innerHTML = `<option value="__NEW__">+ Nuevo ID (Auto-generar)</option>`;

    items.forEach((item) => {
      const pId = item.id || item.playoffId || "PO-0000001";
      const tLabel = item.tournamentName ? ` (${item.tournamentName})` : "";
      const opt = document.createElement("option");
      opt.value = pId;
      opt.textContent = `${pId}${tLabel}`;
    ...
    if (
      currentVal &&
      currentVal !== "__NEW__" &&
      items.some((i) => (i.id || i.playoffId) === currentVal)
    ) {
      idSelect.value = currentVal;
    } else if (
      items.length > 0 &&
      (!idInput || !idInput.value || idInput.value === "PO-0000001")
    ) {
      // Dejar seleccionado __NEW__ por defecto y asegurar el siguiente ID en el input
      idSelect.value = "__NEW__";
      fetchNextPlayOffId();
    }
  }

  /**
   * Sincronización silenciosa en segundo plano del bracket al recuperar foco o visibilidad
   */
  async function syncBracketFromCloudSilently() {
    try {
      ...
      if (res.ok) {
        ...
        if (data && data.bracketData && typeof data.bracketData === "object") {
          const currentStr = JSON.stringify(bracketState);
          const newStr = JSON.stringify(data.bracketData);
        ...
        } else if (data && data.singleRow) {
          const rebuilt = reconstructBracketFromSingleRow(data.singleRow);
          if (rebuilt) {
            localStorage.setItem(
              "KNOCKOUT_BRACKET_STATE",
              JSON.stringify(bracketState),
        ...
        if (changed) {
          renderBracket();
          console.log(
            "[PositionTable.js] Bracket sincronizado silenciosamente con última versión en la nube.",
          );
        }
      }
    } catch (e) {
      // Ignorar errores de sync pasivo
    }
  }

  /**
   * Guarda el estado del bracket tanto en el servidor local (persistente multi-dispositivo)
   * como directamente en la hoja "PlayOffs" de Google Sheets en 1 SOLA FILA por ID (PO-0000001).
   * - Si el ID ya existe: SOBREESCRIBE la fila existente en la hoja PlayOffs.
   * - Si el ID es nuevo: CREA una nueva fila al final de la hoja PlayOffs.
   */
  async function saveBracketState(
    ...
    let chosenId = (idInput ? idInput.value.trim() : "").toUpperCase();
    if (!chosenId || chosenId === "__NEW__") {
      chosenId = "PO-0000001";
    }
    ...
    try {
      ...
      // 1. Construir la estructura exacta en UNA SOLA FILA con todas las llaves y el ID especificado
      const singleRow = buildPlayOffsSingleRow(bracketState, chosenId);
      const playoffRows = [singleRow];
      ...
      // 3. Preparar el payload plano de fila única para Google Apps Script (Hoja PlayOffs)
      const gasPayload = {
        action: "savePlayOffs",
      ...
      let gasAction = "created";
      let gasAssignedId = chosenId;
      ...
      try {
        ...
        if (gasRes.ok) {
          ...
          if (gasData && (gasData.status === "success" || gasData.ok)) {
            gasSuccess = true;
            gasAction = gasData.action || "created";
            if (gasData.playoffId || gasData.id) {
              gasAssignedId = gasData.playoffId || gasData.id;
            }
          }
        }
      } catch (err) {
      ...
      // Actualizar campos de ID tras el guardado
      if (idInput && gasAssignedId) {
        idInput.value = gasAssignedId;
      }
      fetchPlayOffIdsList(false);
      ...
      const actionMsg =
        gasAction === "overwritten"
          ? "sobreescrito y actualizado"
          : "creado con éxito";
      ...
      if (showToastFeedback) {
        if (gasSuccess) {
          ...
          showToast(
            `¡Cuadro de PlayOffs ${actionMsg} en una sola fila en la hoja "${SHEET_CONFIG.PLAYOFFS_SHEET}" con ID "${gasAssignedId}" y sincronizado en la nube!`,
            "success",
          );
        } else {
          updateCloudStatus("success", `Sincronizado en Servidor (${timeStr})`);
          showToast(
            `Cuadro de PlayOffs guardado con éxito (ID: ${gasAssignedId}) en 1 sola fila y sincronizado en el servidor para laptop, PC, tablet y teléfono.`,
            "success",
          );
        }
      }
